...
export default function UsersManager() {
  ...
  return (
    <div className="space-y-8 font-sans relative text-gray-900">
      ...
      <div className="bg-gray-100 border border-gray-200 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          <div className="text-xs font-extrabold text-gray-600 uppercase tracking-wider">
            REGISTERED USERS: <span className="text-gray-900 font-serif font-black">{filteredUsers.length}</span>
          </div>

          <button
            onClick={loadUsers}
            className="px-4 py-2 bg-white border border-gray-300 text-gray-800 text-[10px] font-extrabold tracking-wider uppercase rounded-full hover:bg-gray-50 transition-all cursor-pointer shadow-xs active:scale-95"
          >
            REFRESH USERS
          </button>
        </div>
      ...
      {loading ? (
        <div className="py-24 text-center text-sm text-gray-500 font-medium">Loading user profiles...</div>
        ...
        <div className="space-y-4">
          ...
          <div className="block md:hidden space-y-3.5">
            {filteredUsers.map((u) => {
              const isAdmin = u.role === 'admin';
              return (
                <div key={u.id} className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm space-y-3.5">
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-10 h-10 rounded-full bg-gray-900 text-white font-bold flex items-center justify-center text-sm uppercase shrink-0 shadow-xs">
                        {(u.full_name || u.email || 'U').charAt(0)}
                      </div>
                      <div className="min-w-0">
                        ...
                        <span className="text-xs text-gray-500 block truncate font-medium">{u.email}</span>
                      </div>
                    </div>

                    <span className={`px-2.5 py-1 text-[8.5px] font-black tracking-wider uppercase rounded-full border shrink-0 ${
                      isAdmin
                        ? 'bg-[#C08A3E]/15 text-[#C08A3E] border-[#C08A3E]/30'
                        : 'bg-gray-100 text-gray-600 border-gray-200'
                    }`}>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-gray-100 text-[10px] font-sans">
                    ...
                    <button
                      onClick={() => handleRoleToggle(u)}
                      className={`px-4 py-2 rounded-xl text-xs font-extrabold tracking-wider uppercase transition-all cursor-pointer active:scale-95 ${
                        isAdmin
                          ? 'bg-gray-100 hover:bg-gray-200 text-gray-800 border border-gray-300'
                          : 'bg-gray-900 hover:bg-black text-white shadow-xs'
                      }`}
